<?php
	...
	*/
	...
	function get_tables_info($skip_authentication = false) {
		...
		$all_tables = [
			/* ['table_name' => [table props assoc array] */   
				...
				'Campaigns' => [
					'Caption' => 'Campaigns',
					'Description' => 'A collection of all your fundraising campaigns.',
					'tableIcon' => 'resources/table_icons/32Px - 289.png',
					'group' => $tg[1],
					'homepageShowCount' => 1
				],
				'Donations' => [
				...
				'Notes' => [
					'Caption' => 'Notes',
					'Description' => 'A place to log different notes about your supporters.',
					'tableIcon' => 'resources/table_icons/32Px - 125.png',
					'group' => $tg[0],
					'homepageShowCount' => 0
				],
				'MatchingFunds' => [
					'Caption' => 'Matching Funds',
					'Description' => 'Track your matching funds.',
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'Supporters' => ['Supporters', 'A collection of all your supporters.', 'resources/table_icons/32Px - 002.png', 'Supporters'],
			'Campaigns' => ['Campaigns', 'A collection of all your fundraising campaigns.', 'resources/table_icons/32Px - 289.png', 'Donations'],
		...
		if($schema === null) {
			...
			$schema = [
				'Supporters' => [
					...
					'Country' => [
						'appgini' => "VARCHAR(40) NULL DEFAULT 'United States'",
						'info' => [
							'caption' => 'Country',
							'description' => '',
						],
				...
				'Campaigns' => [
					...
					'Goal' => [
						...
						'info' => [
							'caption' => 'Goal',
							'description' => 'What is the goal you have setup for your campaign?',
						],
					],
					'DateClosed' => [
						'appgini' => "DATETIME NULL",
						'info' => [
							'caption' => 'Date Closed',
							'description' => 'When was the campaign officially closed. ',
						],
					],
					'Status' => [
						'appgini' => "VARCHAR(10) NULL",
						'info' => [
							'caption' => 'Status',
							'description' => 'Is your campaign complete?',
						],
					],
				],
				'Donations' => [
					...
					'SupporterID' => [
						...
						'info' => [
							'caption' => 'Supporter',
							'description' => 'This is to link the donation to a supporter.',
						],
					],
					'CampaignID' => [
						...
						'info' => [
							'caption' => 'Campaign',
							'description' => 'This is to identify what Campaing the donatin should be associated with.  If there isn\'t one there should be a general campaig value you can assign.',
						],
					],
					'Paytype' => [
						'appgini' => "TEXT NULL",
					...
					'Matching' => [
						...
						'info' => [
							'caption' => 'Matching',
							'description' => 'Is the donation going to be Matched?',
						],
					],
					'Anonymous' => [
						'appgini' => "INT NULL",
						'info' => [
							'caption' => 'Anonymous',
							'description' => 'Is this an Anonymous Donation?',
						],
					],
					'Acknowledged' => [
						'appgini' => "INT NULL",
						'info' => [
							...
							'description' => 'Has an Acknowledgement letter been sent?',
						],
					],
					'Notes' => [
						'appgini' => "TEXT NULL",
					...
					'MemoryOf' => [
						...
						'info' => [
							...
							'description' => 'Is the donation being made in memory of someone?',
						],
					],
					'HonorOf' => [
						'appgini' => "VARCHAR(40) NULL",
						'info' => [
							'caption' => 'In Honor Of',
							'description' => 'Is the donation being made in honor of someone?',
						],
					],
				],
				'Notes' => [
				...
				'MatchingFunds' => [
					...
					'Attachment' => [
						...
						'info' => [
							'caption' => 'Attachment',
							'description' => 'Maximum file size allowed: 8000 KB.<br>Allowed file types: txt, doc, docx, docm, odt, pdf, rtf',
						],
					],
				],
			];
	...
	*/
	...
	function calculated_fields() {
		...
		return [
			'Supporters' => [
				'TotalDonated' => 'SELECT CONCAT(\'$\', FORMAT(sum(`Donations`.`Amount`),2)) FROM `Supporters`
					
					LEFT JOIN `Donations` ON `Donations`.`SupporterID`=`Supporters`.`ID` 
					 
					WHERE `Supporters`.`ID`=\'%ID%\'',
				'TotalMatched' => 'SELECT CONCAT(\'$\', FORMAT(SUM(`MatchingFunds`.`Amount`),2)) FROM `Supporters`
					
					LEFT JOIN `MatchingFunds` ON `MatchingFunds`.`SupporterID`=`Supporters`.`ID` 
			...
			'Campaigns' => [],
			'Donations' => [
				'DateLink' => 'SELECT CAST(`Donations`.`Date` as DATE) FROM `Donations`
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'Donations' => [
				'SupporterID' => 'SELECT `Supporters`.`ID`, IF(CHAR_LENGTH(`Supporters`.`LastName`) || CHAR_LENGTH(`Supporters`.`Name`), CONCAT_WS(\'\', `Supporters`.`LastName`, \', \', `Supporters`.`Name`), \'\') FROM `Supporters` ORDER BY 2',
			...
			'MatchingFunds' => [
				'SupporterID' => 'SELECT `Supporters`.`ID`, IF(CHAR_LENGTH(`Supporters`.`LastName`) || CHAR_LENGTH(`Supporters`.`Name`), CONCAT_WS(\'\', `Supporters`.`LastName`, \', \', `Supporters`.`Name`), \'\') FROM `Supporters` ORDER BY 2',
				'DonationID' => 'SELECT `Donations`.`ID`, IF(CHAR_LENGTH(if(`Donations`.`DateLink`,date_format(`Donations`.`DateLink`,\'%m/%d/%Y\'),\'\')) || CHAR_LENGTH(`Donations`.`Amount`), CONCAT_WS(\'\', if(`Donations`.`DateLink`,date_format(`Donations`.`DateLink`,\'%m/%d/%Y\'),\'\'), \' - \', `Donations`.`Amount`), \'\') FROM `Donations` LEFT JOIN `Supporters` as Supporters1 ON `Supporters1`.`ID`=`Donations`.`SupporterID` LEFT JOIN `Campaigns` as Campaigns1 ON `Campaigns1`.`ID`=`Donations`.`CampaignID` ORDER BY 2',
			],
		];
